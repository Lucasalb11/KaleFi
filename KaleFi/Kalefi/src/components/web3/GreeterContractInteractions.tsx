  ...
  const fetchGreeting = useCallback(async () => {
    ...
    if (!currentChain) {
      ...
      try {
        ...
        const result_string = StellarSdk.scValToNative(
          result as StellarSdk.xdr.ScVal
        ) as string
    ...
    return (
      <div tw="flex grow flex-col space-y-4 max-w-[20rem]">
        ...
        <Card variant="outline" p={4} bgColor="whiteAlpha.100">
          <p tw="text-center font-mono text-sm">
            No deployment found in the current chain
          </p>
          <p tw="mt-4 text-center font-mono text-lg">Available deployments:</p>
          {sorobanContext?.deployments?.map((d, i) => (
            <p key={i} tw="text-center font-mono text-sm">
              - {d.networkPassphrase}
            </p>
          ))}
        </Card>
      ...
      <div tw="flex grow flex-col space-y-4 max-w-[20rem]">
        ...
        <Card variant="outline" p={4} bgColor="whiteAlpha.100">
          <FormControl>
            ...
            <Input placeholder={fetchedGreeting} disabled={true} />
          </FormControl>
        </Card>
