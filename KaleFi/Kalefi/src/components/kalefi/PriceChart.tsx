...
export const PriceChart: React.FC = () => {
  ...
  return (
    <div tw="rounded-lg border border-gray-700 bg-gray-800 p-6">
      <div tw="mb-4 flex items-center justify-between">
        <div tw="flex items-center gap-2">
          <FaChartLine className="text-blue-400" />
          <h3 tw="font-semibold text-lg text-white">KALE Price Chart</h3>
        </div>
        <div tw="gap-2">
          {[1, 7, 30].map((days) => (
            <button
      ...
      <div tw="mb-4 rounded-lg bg-gray-700 p-3">
      ...
      {/* Chart Visualization */}
      <div tw="relative h-48 rounded-lg bg-gray-700 p-4">
        {isLoading ? (
          <div tw="flex h-full items-center justify-center">
            <div tw="h-8 w-8 animate-spin rounded-full border-2 border-blue-400 border-t-transparent"></div>
          </div>
        ) : chartData.length > 0 ? (
                ...
                points={chartData
                  .map((point, index) => {
                    ...
                    const y =
                      100 -
                      (point.price /
                        Math.max(...chartData.map((p) => p.price))) *
                        80
                    return `${x},${y}`
                  })
                  .join(' ')}
          ...
          <div tw="flex h-full items-center justify-center text-gray-400">
            No data available
          </div>
        )}
      </div>
      ...
      {/* Data Points */}
      <div tw="mt-4 grid grid-cols-2 gap-2 text-xs md:grid-cols-4">
        {chartData.slice(-4).map((point, index) => (
          <div key={index} tw="rounded bg-gray-700 p-2 text-center">
            <div tw="text-gray-400">{formatDate(point.timestamp)}</div>
            <div tw="font-medium text-white">${point.price.toFixed(4)}</div>
          </div>
        ))}
      </div>
    </div>
