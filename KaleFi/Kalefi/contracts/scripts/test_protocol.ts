...
export async function testProtocol(network: string) {
  ...
  try {
    ...
    const kaleBalanceResult = await invoke(
      kaleBalanceOp,
      loadedConfig.admin,
      true
    )
    const kaleBalance = parseInt(
      scValToNative(kaleBalanceResult.result.retval).toString()
    )
    ...
    const usdcBalanceResult = await invoke(
      usdcBalanceOp,
      loadedConfig.admin,
      true
    )
    const usdcBalance = parseInt(
      scValToNative(usdcBalanceResult.result.retval).toString()
    )
    console.log(`Admin USDC Balance: ${usdcBalance / 1000000} USDC`)

    // Check KaleFi contract USDC balance
    const contractUsdcOp = usdcContract.call(
      'balance',
      new Address(kalefiId).toScVal()
    )
    const contractUsdcResult = await invoke(
      contractUsdcOp,
      loadedConfig.admin,
      true
    )
    const contractUsdcBalance = parseInt(
      scValToNative(contractUsdcResult.result.retval).toString()
    )
    console.log(
      `KaleFi Contract USDC Balance: ${contractUsdcBalance / 1000000} USDC\n`
    )
    ...
    const healthOp = kalefiContract.call(
      'check_health_factor',
      adminAddress.toScVal()
    )
    ...
    const finalKaleBalance = parseInt(
      scValToNative(finalKaleResult.result.retval).toString()
    )
    ...
    const finalUsdcBalance = parseInt(
      scValToNative(finalUsdcResult.result.retval).toString()
    )
    console.log(`Final Admin USDC Balance: ${finalUsdcBalance / 1000000} USDC`)

    // Check final health factor
    const finalHealthOp = kalefiContract.call(
      'check_health_factor',
      adminAddress.toScVal()
    )
    const finalHealthResult = await invoke(
      finalHealthOp,
      loadedConfig.admin,
      true
    )
    ...
    if (Array.isArray(finalHealthData)) {
      ...
      console.log(
        `Final Collateral Value: ${finalCollateralValue / 10000000} USD`
      )
